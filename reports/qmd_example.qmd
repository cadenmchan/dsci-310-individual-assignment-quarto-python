...
![Horse Populations](../results/horse_pops_plot.png){#fig-populations width=65% height=65%}

@fig-populations: Horse populations for all provinces in Canada from 1906 - 1972.
...
We present the standard deviations in @tbl-stdev.

[@tbl-stdev]. Standard deviation of historical (1906-1972) horse populations for each Canadian province.

```{python}
# | label: tbl-stdev
# | tbl-cap: Horse Population Standard Deviation (1906-1972)
...
Additionally, note that in @tbl-stdev we consider the sample standard deviation of the number of horses during the same time span as @fig-populations.

![Saskatchewan Horse Population](../results/horse_pops_plot_largest_sd.png){#fig-sask-horse-pop width=65% height=65%}

@fig-sask-horse-pop: Horse populations for the province with the largest standard deviation

In @fig-sask-horse-pop we zoom in and look at the province of `{python} largest_sd`, which had the largest spread of values in terms of standard deviation.
